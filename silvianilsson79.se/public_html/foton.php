<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <title>Silvia Nilsson</title>
        <link rel="stylesheet" href="css/style.css" />
        <link href="css/lightbox.css" rel="stylesheet" />
        
        <script src="js/jquery-1.7.2.min.js"></script>
        <script src="js/lightbox.js"></script>

    </head>
    
    <body>
      
        <section id="page_container">
           <section id="head">
            	<section id="head-left">
           			<header>
           				<a href="index.html"><h1>Silvia Nilsson</h1></a>
           			</header>
                </section>
                <section id="head-right">
                    <ul id="list-nav">
                        <li><a href="index.html">Hem</a></li>
                        <li><a href="ommig.html">Om mig</a></li>
                        <li><a href="kontakt.html">Kontakt</a></li>
                        <li class="selected"><a href="foton.html">Foton</a></li>
                        <li><a href="links.html">Länkar</a></li>
                    </ul>
                </section>
           </section>
            
            <section id="center_column"> 
            <a href="./images/big/lightbox01.jpg" rel="lightbox[roadtrip]" title="Vår kanin Silledanke"><img class="foto" src="./images/mini/lightbox01.jpg" alt="Silledanke" title="Silledanke"></img></a>
            <a href="./images/big/lightbox02.jpg" rel="lightbox[roadtrip]" title="Kålmorden"><img class="foto" src="./images/mini/lightbox02.jpg" alt="Kålmorden" title="Kålmorden"></img></a>
            <a href="./images/big/lightbox03.jpg" rel="lightbox[roadtrip]" title="Skara Sommarland"><img class="foto" src="./images/mini/lightbox03.jpg" alt="SkaraSommarland" title="SkaraSommarland"></img></a>
            <a href="./images/big/lightbox04.jpg" rel="lightbox[roadtrip]" title="Turkiet utanför vårt hotell"><img class="foto1" src="./images/mini/lightbox04.jpg" alt="Turkiet" title="Turkiet"></img></a>
            <a href="./images/big/lightbox05.jpg" rel="lightbox[roadtrip]" title="Turkiet"><img class="foto1" src="./images/mini/lightbox05.jpg" alt="Turkiet" title="Turkiet"></img></a>
            <a href="./images/big/lightbox06.jpg" rel="lightbox[roadtrip]" title="Marcus på scen i Turkiet"><img class="foto1" src="./images/mini/lightbox06.jpg" alt="Turkiet" title="Turkiet"></img></a>
            <a href="./images/big/lightbox07.jpg" rel="lightbox[roadtrip]" title="Eric Saade på besök i Växjö"><img class="foto2" src="./images/mini/lightbox07.jpg" alt="EricSaade" title="EricSaade"></img></a>
            <a href="./images/big/lightbox08.jpg" rel="lightbox[roadtrip]" title="Silledanke"><img class="foto2" src="./images/mini/lightbox08.jpg" alt="Silledanke" title="Silledanke"></img></a>
            <a href="./images/big/lightbox09.jpg" rel="lightbox[roadtrip]" title="Besökte Universium"><img class="foto2" src="./images/mini/lightbox09.jpg" alt="Universeum" title="Universium"></img></a>
            <a href="./images/big/lightbox10.jpg" rel="lightbox[roadtrip]" title="Universium"><img class="foto3" src="./images/mini/lightbox10.jpg" alt="Universeum" title="Universeum"></img></a>
            <a href="./images/big/lightbox11.jpg" rel="lightbox[roadtrip]" title="Ska åka helikopter"><img class="foto3" src="./images/mini/lightbox11.jpg" alt="Karl-oskar" title="Karl-oskar"></img></a>
            <a href="./images/big/lightbox12.jpg" rel="lightbox[roadtrip]" title="Blomma"><img class="foto3" src="./images/mini/lightbox12.jpg" alt="Blomma" title="Blomma"></img></a>
                
                
        	</section>
            
        	<section id="right_column">
            	
            </section>
           
           
            <section id="footer">
            	<p> © Silvia Nilsson 2012 . All rights reserved</p>
                
            </section>
       	
         
        
    </body>
</html>